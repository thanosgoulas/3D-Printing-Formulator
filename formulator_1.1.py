...
#  formulator_1.1.py
#
#  The 3D Printing Formulator  —  v1.1
...
DEFAULT_MATERIALS = [
    ...
    {"acronym": "PPG-200",                           "name": "Polypropylene Glycol 200",                                    "density": 1.005, "ri": 1.4510, "mw": 200.0},
    {"acronym": "PPG-400",                           "name": "Polypropylene Glycol 400",                                    "density": 1.010, "ri": 1.447, "mw": 400.0},
...
"el": {  # Greek
    ...
    "by Dr Thanos Goulas  ·  v1.1": "Copyright © 2026 Dr Thanos Goulas  ·  v1.1",
...
class MaterialsDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        _btn(bf2, _T("🔀 Merge DB"),  self._merge_defaults, "neutral").pack(side="left", padx=(6,0))
    ...
    def _merge_defaults(self):
        """Import a default materials JSON, adding only NEW entries.
        Existing user entries are never modified or deleted.
        Use this when upgrading to a new version to get new default materials.
        """
        from tkinter import filedialog
        path = filedialog.askopenfilename(
            parent=self,
            title="Merge Default Materials — select a default_materials_*.json file",
            filetypes=[("JSON files", "*.json"), ("All files", "*.*")])
        if not path: return
        try:
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            if not isinstance(data, list) or not all(
                    isinstance(m, dict) and "acronym" in m and "density" in m
                    for m in data):
                messagebox.showerror("Merge",
                    "File does not appear to be a valid materials database.",
                    parent=self)
                return

            existing_acronyms = {e["acronym"] for e in self._db}
            new_entries = [_migrate_entry(m) for m in data
                           if _migrate_entry(m)["acronym"] not in existing_acronyms]

            if not new_entries:
                messagebox.showinfo("Merge DB",
                    "No new materials found.\n\n"
                    "All entries in the selected file already exist in your database.",
                    parent=self)
                return

            msg = (f"Found {len(new_entries)} new material(s) to add:\n\n"
                   + "\n".join(f"  • {m['acronym']}" for m in new_entries[:15])
                   + (f"\n  ... and {len(new_entries)-15} more" if len(new_entries) > 15 else "")
                   + "\n\n"
                   "Your existing entries will NOT be modified.\n\n"
                   "Proceed?")
            if not messagebox.askyesno("🔀 Merge DB", msg, parent=self):
                return
            ...
            messagebox.showinfo("Merge Complete",
                f"{len(new_entries)} new material(s) added to your database.\n\n"
                "Your existing entries were not modified.",
                parent=self)
        except Exception as e:
            messagebox.showerror("Merge", str(e), parent=self)
    ...
    def _save(self):
        ...
        if existing:
            if not messagebox.askyesno("Overwrite",
                    ...
                    parent=self):
                return
            existing["density"] = round(rho_blend, 4)
            existing.setdefault("acronym", name)
            existing.setdefault("ri", None)
            existing.setdefault("mw", None)
            n = self._n_rows.get()
            existing["blend_components"] = [
                {"acronym": self._names[i].get().strip(),
                 "wt_pct":  float(self._wts[i].get().strip())}
                for i in range(n)
                if self._names[i].get().strip() and self._wts[i].get().strip()
            ]
        else:
            n = self._n_rows.get()
            blend_comps = [
            ...
            self._db.append({"acronym": name, "name": "", "density": round(rho_blend, 4),
                             "ri": None, "mw": None, "blend_components": blend_comps})
        self._db.sort(key=lambda m: m["acronym"].lower())
        save_materials_db(self._db)
        if self._on_created:
            self._on_created()
...
def _resolve_blend_components(comp_name, materials_db):
    """Return a list of {acronym, wt_pct} dicts for a blend, or None.

    Strategy:
      1. Look for blend_components stored in the DB entry (new blends).
      2. Parse the blend name itself — the auto-generated name format is
         "MAT1 X% / MAT2 Y%" or "MAT1 X.Y% / MAT2 Z%".
         Each token is tested against all DB acronyms.
    Returns None if the component is not a blend or cannot be resolved.
    """
    import re
    mat = next((m for m in materials_db if m.get("acronym") == comp_name), None)
    if not mat:
        return None

    # Route 1: explicit composition stored in DB entry
    stored = mat.get("blend_components")
    if stored:
        return stored

    # Route 2: parse the auto-generated blend name
    # Format: "ACMO 10% / HDDA 45% / TMPTA 45%"
    #      or "HDDA 50% / TMPTA 50%"
    #      or "HDDA 50.0% / TMPTA 50.0%"
    db_acronyms = {m["acronym"] for m in materials_db}
    parts = [p.strip() for p in comp_name.split("/")]
    if len(parts) < 2:
        return None   # not a blend name pattern

    parsed = []
    for part in parts:
        # Match "ACRONYM NUMBER%" — acronym may contain letters, digits, -, (, )
        m = re.match(r"^(.+?)\s+([\d.]+)\s*%\s*$", part.strip())
        if not m:
            return None
        acronym = m.group(1).strip()
        wt_pct  = float(m.group(2))
        if acronym not in db_acronyms:
            return None   # part not found in DB — can't resolve
        parsed.append({"acronym": acronym, "wt_pct": wt_pct})

    # Sanity check: wt% should sum to ~100
    total = sum(p["wt_pct"] for p in parsed)
...
def _attach_blend_tooltip(tree, tab):
    """Show individual blend component masses on hover over any cell
    in a blend row, after Solve/Calculate has been clicked.

    Uses winfo_pointerxy polling via after() instead of <Motion> binding,
    which is more reliable on Windows with ttk.Treeview.
    """
    tip_win  = [None]
    last_row = [None]
    poll_id  = [None]
    ...
    def _show_for_row(row_id, mx, my):
        db = tab.app.materials_db
        try:
            idx = int(row_id)
        except ValueError:
            return
        if idx >= len(tab.components):
            return

        comp      = tab.components[idx]
        comp_name = (comp.get("name", "") if isinstance(comp, dict)
                     else getattr(comp, "name", ""))
        if not comp_name:
            return

        # Resolve blend live from DB — works for both new and loaded recipes
        blend_comps = _resolve_blend_components(comp_name, db)
        if not blend_comps:
            return

        # Mass from tree row (column index 6)
        values = tree.item(row_id, "values")
        if not values or len(values) < 7:
            return
        mass_str = str(values[6]).replace(" g", "").strip()
        if not mass_str or mass_str == "--":
            return
        try:
        ...
        # Build tooltip window with grid layout for proper alignment
        tw = tk.Toplevel(tree)
        tw.wm_overrideredirect(True)
        tw.attributes("-topmost", True)
        tw.configure(bg=BORDER_MED)
        tw.wm_geometry(f"+{mx + 18}+{my + 14}")

        outer = tk.Frame(tw, bg=BG_WHITE, padx=10, pady=8)
        outer.pack()

        # Header
        tk.Label(outer, text=f"Blend breakdown",
        ...
        # Separator line
        tk.Frame(outer, bg=BORDER_MED, height=1).grid(
            row=1, column=0, columnspan=3, sticky="ew", pady=(0, 4))

        # Data rows
        for i, bc in enumerate(blend_comps):
            ind_mass = total_mass * bc["wt_pct"] / 100.0
            tk.Label(outer, text=bc["acronym"],
                     bg=BG_WHITE, fg=TEXT,
                     font=("Segoe UI", 9),
                     anchor="w").grid(row=i+2, column=0,
                                      sticky="w", padx=(0, 16))
            tk.Label(outer, text=f"{bc['wt_pct']:.1f} wt.%",
                     bg=BG_WHITE, fg=TEXT_DIM,
                     font=("Segoe UI", 9),
                     anchor="e").grid(row=i+2, column=1,
                                      sticky="e", padx=(0, 12))
            tk.Label(outer, text=f"{ind_mass:.2f} g",
                     bg=BG_WHITE, fg=TEXT,
                     font=("Segoe UI", 9, "bold"),
                     anchor="e").grid(row=i+2, column=2, sticky="e")

        # Separator + total
        n = len(blend_comps)
        tk.Frame(outer, bg=BORDER_MED, height=1).grid(
            row=n+2, column=0, columnspan=3, sticky="ew", pady=(4, 4))
        tk.Label(outer, text="TOTAL",
                 bg=BG_WHITE, fg=TEXT,
                 font=("Segoe UI", 9, "bold"),
                 anchor="w").grid(row=n+3, column=0, sticky="w", padx=(0, 16))
        tk.Label(outer, text="100.0 wt.%",
                 bg=BG_WHITE, fg=TEXT_DIM,
                 font=("Segoe UI", 9),
                 anchor="e").grid(row=n+3, column=1, sticky="e", padx=(0, 12))
        tk.Label(outer, text=f"{total_mass:.2f} g",
                 bg=BG_WHITE, fg=ACCENT,
                 font=("Segoe UI", 9, "bold"),
                 anchor="e").grid(row=n+3, column=2, sticky="e")
    ...
    def _poll():
        try:
            ...
            # Get pointer position relative to screen
            mx = tree.winfo_pointerx()
            my = tree.winfo_pointery()

            # Convert to tree-local coordinates
            tx = mx - tree.winfo_rootx()
            ty = my - tree.winfo_rooty()

            # Check if pointer is inside the tree widget
            inside = (0 <= tx < tree.winfo_width() and
                      0 <= ty < tree.winfo_height())

            if not inside:
                _hide()
            else:
                row_id = tree.identify_row(ty)
                if not row_id or row_id == "total":
                    _hide()
                elif row_id != last_row[0]:
                    _hide()
                    last_row[0] = row_id
                    _show_for_row(row_id, mx, my)
                # If same row, leave existing tooltip in place

        except Exception:
            pass
        finally:
            # Reschedule — 120ms is smooth without being CPU-hungry
            poll_id[0] = tree.after(120, _poll)

    # Expose reset — called from _refresh() after load/clear so the
    # tooltip re-evaluates even if the cursor hasn't moved.
    def _reset_cache():
        _hide()
        last_row[0] = "__reset__"   # set AFTER _hide so it is not cleared
    tree._blend_tip_reset = _reset_cache

    # Start polling when tree is realised
    tree.after(500, _poll)
    tree.bind("<Destroy>", lambda _: tree.after_cancel(poll_id[0])
              if poll_id[0] else None)
...
class ForwardTab(tk.Frame):
    ...
    def _build(self):
        ...
        b=_btn(rhs2,_T("📄 New"),       lambda:self.app.new_recipe(self),"neutral"); b.pack(side="right",padx=(2,0)); _tip(b,"Start a new formulation.")
        ...
        _attach_blend_tooltip(self.tree, self)
    ...
    def _refresh(self,result=None):
        ...
        if hasattr(self.tree, "_blend_tip_reset"):
            self.tree._blend_tip_reset()
...
class InverseTab(tk.Frame):
    ...
    def _build(self):
        ...
        b=_btn(rhs,_T("📄 New"),       lambda:self.app.new_recipe(self),"neutral"); b.pack(side="right",padx=(2,0)); _tip(b,"Start a new formulation. Prompts to save if current recipe has components.")
    ...
    def _refresh(self,result=None):
        self.tree.delete(*self.tree.get_children())
        # Reset blend tooltip cache so hover works after load/clear
        if hasattr(self.tree, "_blend_tip_reset"):
            self.tree._blend_tip_reset()
        fmt_mv = "{:.2f}"
        res_map={r["name"]:r for r in result["rows"]} if result else {}
...
class FormulatorApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(sb, text="Copyright 2026  ©  ·  Dr Thanos Goulas  ·  v1.1", bg=BG_WHITE, fg=TEXT_MUTED,
    ...
    def new_recipe(self, calling_tab):
        """Clear everything and start a fresh formulation.
        Prompts to save first if the current recipe has components.
        """
        fwd_has = bool(self.fwd_tab.components)
        inv_has = bool(self.inv_tab.components)
        has_work = fwd_has or inv_has

        if has_work:
            recipe_name = self.var_recipe_name.get().strip() or 'this recipe'
            msg = 'Do you want to save ' + repr(recipe_name) + ' before starting a new formulation?'
            answer = messagebox.askyesnocancel(
                _T("New Recipe"), msg, parent=self)
            if answer is None:
                return
            if answer:
                calling_tab._save()
        ...
        # Clear both tabs
        for tab in (self.fwd_tab, self.inv_tab):
            tab.components.clear()
            tab.last_result = None
            tab._refresh()
            tab._reset_summary()

        # Reset recipe name and inverse target
        self.var_recipe_name.set('')
        self.inv_tab.var_target.set('45')
        self.set_status(_T("New recipe started."))
...
To back up or share your database:
  ...
  Import:  In the database editor → click 📥 Import DB.
           ...
           Use this to bring in data from a colleague's database.

  Merge DB:  In the database editor → click 🔀 Merge DB.
           Adds ONLY new entries from a default_materials_*.json file.
           Your existing entries are NEVER modified or deleted.
           Use this when upgrading to a new version of the app
           to receive new default materials without losing your
           custom entries.
...
class SettingsDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(info,
                 text="The 3D Printing Formulator  ·  v1.1\n"
